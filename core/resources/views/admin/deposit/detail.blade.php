        ...
        @if ($deposit->sendMoney && $deposit->status != Status::PAYMENT_INITIATE)
            <div class="col-12">
                <div class="alert alert-info p-3">
                    <p class="f-size--24">
                        <a href="{{ route('admin.users.detail', $deposit->user_id) }}" class="f-size--24">
                            {{ @$deposit->user->username }}
                        </a>
                        @lang('have paid')
                        <span class="fw-bold text--primary">{{ showAmount($deposit->amount) }}</span>
                        @lang('to send')
                        <span class="fw-bold">
                            {{ showAmount($deposit->sendMoney->sending_amount, currencyFormat: false) }}
                            {{ $deposit->sendMoney->sending_currency }}
                        </span>
                        @lang('to')
                        {{ @$deposit->sendMoney->recipientCountry->name }}
                        @lang('from')
                        {{ @$deposit->sendMoney->sendingCountry->name }}.
                        @lang('The recipient will receive')
                        <span class="fw-bold">
                            {{ showAmount($deposit->sendMoney->recipient_amount, currencyFormat: false) }}
                            {{ $deposit->sendMoney->recipient_currency }}
                        </span>
                    </p>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            ...
                            @else
                                <span class="fw-bold">
                                    <a href="{{ route('admin.agents.detail', $deposit->agent_id) }}">
